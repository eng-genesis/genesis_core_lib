...
Here's a comprehensive example showing how to create a complete synthetic data generation pipeline using library classes:
...
# 2. Pick metadata for model
metadata = dataset.to_skeleton()
input_shape = dataset.get_shape_for_model()
...
# 3. Create Processor with step strategy
processor = TableProcessor(save_filepath="./models")
...
2. **Model Instantiation**: `TabularVAE()` creates the model with specific hyperparameters
3. **Processor Setup**: `TableProcessor` with `VAEStrategy` handles data preprocessing
4. **Training**: Model learns from preprocessed data using `model.train()`
